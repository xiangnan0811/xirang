...
- **节点管理** — SSH 接入多台服务器，实时监控 CPU / 内存 / 磁盘
- **备份策略** — 灵活配置 cron 定时备份，支持模板和批量应用
- **任务编排** — 远程命令执行、批量操作、任务依赖链、失败自动重试
- **文件浏览** — SFTP 远程文件浏览与本地备份目录管理
- **Web 终端** — 浏览器内直接 SSH 连接服务器
- **安全** — RBAC 权限控制、TOTP 两步验证、登录锁定、敏感字段加密
- **SLA 报告** — 可配置的定期运维报告自动生成
