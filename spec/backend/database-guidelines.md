...
## Installation-stable key domains

`entry_identity`, `recovery_cleanup_ownership` and `search_token` keys are
installation-stable: `Keyring.Rotate` rejects all three domains. Application KEK
rotation only rewraps their envelopes without changing the domain key material.
Do not silently replace a lost key. Entry IDs underpin existing deep links and
overlay references; a future replacement needs an explicit compatibility design.

Keep key domains separate. Search projections can be rebuilt under their existing
publication/revocation contracts without rotating `search_token` or changing its
HMAC tokens. Derived and Export stores follow their own coordinated invalidation
contracts; those paths do not authorize rotating installation-stable keys. See
`backend/internal/backupasset/keyring.go` (`Ensure`, `Rotate`, `Rewrap`), its
domain-specific regression tests, and the Search Token contract below.
