...
浏览器测试的 `Storage`、`localStorage` 和 `sessionStorage` 必须来自同一个
jsdom 实例，并在 MSW、i18n 等读取存储的模块初始化之前安装。Node 自带的 Web Storage
不能混入浏览器测试，也不使用普通对象兜底替换原型语义。调整测试环境后，须在 CI
使用的 Node 版本及触发问题的本地 Node 版本验证存储与受影响的 Hook 回归。
