...
升级到包含 `000083_task_run_recovery_capture`、`000084_alert_delivery_intents` 的版本时，先暂停新任务准入并排空、停止所有旧 Core，再让新 Core 执行迁移；不要让不理解捕获代次或投递认领的旧进程继续写入同一数据库。
...
- 已使用的捕获、代次或投递证据会阻止这两项 schema 降级。不要删除证据、修改迁移版本或强行混用旧二进制来绕过保护。回退方案必须同时保全数据库、可变备份树和通知回执状态，而不只是替换镜像。
