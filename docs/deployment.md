...
升级到包含 `000083_task_run_recovery_capture`、`000084_alert_delivery_intents` 或 `000085_alert_delivery_success` 的版本时，先备份数据库及加密密钥，暂停新任务准入并排空、停止所有旧 Core，再让新 Core 执行迁移；不要让不理解捕获代次、投递认领或成功回执的旧进程继续写入同一数据库。
...
- 已使用的捕获、代次或投递证据会阻止相应 schema 降级；`000085` 写入发送成功时间或未知投递身份后也会阻止降级。不要删除证据、修改迁移版本或强行混用旧二进制来绕过保护。优先前向修复；回退方案必须同时保全数据库、可变备份树和通知回执状态，而不只是替换镜像。
