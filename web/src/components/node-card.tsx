...
export type NodeCardProps = {
  ...
  onToggleSelection: (nodeId: number, checked: boolean) => void;
  onTestNode: (node: NodeRecord) => void;
  onDeleteNode: (node: NodeRecord) => void;
  onEditNode: (node: NodeRecord) => void;
  onTriggerBackup: (nodeId: number, nodeName: string) => void;
...
export function NodeCard({
  ...
  return (
    <div
      ...
      className={cn(
        "group rounded-lg border border-border bg-card shadow-sm hover:shadow-md transition-shadow p-3 outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:border-transparent",
    ...
    >
      ...
      {/* Main info — polished: name large + semibold, IP/tag small muted */}
      <div className="mt-2">
      ...
      <div className="mt-2 space-y-1 text-xs text-muted-foreground">
        ...
        <p>
          {t("nodes.diskFreeDetail", {
            pct: node.diskFreePercent,
            latency: node.connectionLatencyMs ? `${node.connectionLatencyMs}ms` : "-",
          })}
        </p>
        ...
        {node.tags.length > 0 && (
          <p className="break-words text-xs text-muted-foreground">
            {t("nodes.tagsLabel", { tags: node.tags.join(" / ") })}
          </p>
        )}
      </div>
      ...
      <div className="mt-4 flex flex-wrap-reverse items-center justify-between gap-2 border-t border-border pt-3">
        <div className="flex flex-wrap items-center gap-1">
          <Button
            ...
            aria-label={t("nodes.testConnectionAriaLabel", { name: node.name })}
            title={t("nodes.testConnection")}
          ...
          <Button
            ...
            size="icon"
            className="size-8 text-muted-foreground hover:bg-accent hover:text-foreground"
            aria-label={t("nodes.viewLogsAriaLabel", { name: node.name })}
          ...
          {isAdmin && (
            <Button
              ...
              size="icon"
              className="size-8 text-muted-foreground hover:bg-accent hover:text-foreground"
              aria-label={t("nodes.webTerminalAriaLabel", { name: node.name })}
              title={t("nodes.webTerminal")}
          ...
          <Button
            ...
            size="icon"
            className="size-8 text-muted-foreground hover:bg-accent hover:text-foreground"
            aria-label={t("nodes.fileBrowserAriaLabel", { name: node.name })}
            title={t("nodes.fileBrowser")}
          ...
          <Button
            variant="ghost"
            size="icon"
            className="size-8 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100 hover:bg-accent hover:text-foreground focus-visible:opacity-100"
            aria-label={t("nodes.editNodeAriaLabel", { name: node.name })}
            title={t("nodes.editNode")}
          ...
          <Button
            variant="ghost"
            size="icon"
            className="size-8 text-destructive/80 opacity-0 transition-opacity group-hover:opacity-100 hover:bg-destructive/10 hover:text-destructive focus-visible:opacity-100"
            aria-label={t("nodes.deleteNodeAriaLabel", { name: node.name })}
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            onClick={() => onTriggerBackup(node.id, node.name)}
          >
            {triggeringNodeId === node.id && (
...
/** Mobile-only simplified card (no migrate/emergency, just basic actions) */
export function NodeCardMobile({
  node,
  checked,
  testingNodeId,
  triggeringNodeId,
...
>) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-3 shadow-sm">
      ...
      <div className="mt-2 space-y-1 text-xs text-muted-foreground">
        <p>
          {t("nodes.diskFreeLabel", {
            pct: node.diskFreePercent,
            probe: node.diskProbeAt || t("nodes.probeNever"),
          })}
        </p>
        <p>{t("nodes.lastBackupLabel", { time: node.lastBackupAt })}</p>
