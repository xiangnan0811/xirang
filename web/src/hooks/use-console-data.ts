...
function deriveOverview(nodes: NodeRecord[], policies: PolicyRecord[], tasks: TaskRecord[]): OverviewStats {
...
function buildFingerprint(privateKey: string) {
...
function createKeyId(name: string) {
  return `key-${name.toLowerCase().replace(/\s+/g, "-")}-${Date.now().toString(36)}`;
}

function createIntegrationId(name: string) {
