...
CREATE TABLE recovery_point_lifecycle_attempts_new (
    ...
    blocked_reason TEXT NOT NULL DEFAULT '' CHECK (blocked_reason IN ('', 'active_hold', 'lease_live', 'lease_drain_unproven', 'owner_cleanup_unproven', 'provider_worm', 'provider_unavailable', 'provider_identity_conflict', 'provider_delete_unproven', 'deletion_unavailable', 'fence_lost', 'provider_native_version_referenced')),
    claimed_at DATETIME,
...
BEGIN
    SELECT RAISE(ABORT, '000076 downgrade blocked: provider native version reference reason exists');
END;
