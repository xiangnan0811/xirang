...
CREATE TABLE backup_asset_delivery_grants_new (
    ...
    renderer TEXT NOT NULL CHECK (renderer IN ('escaped_text', 'plain_text', 'safe_raster', 'same_origin_pdf', 'native_audio', 'native_video', 'metadata_hex', 'attachment')),
    profile TEXT NOT NULL CHECK (profile IN ('text_v1', 'text_v2', 'raster_v1', 'pdf_v1', 'audio_v1', 'video_v1', 'hex_v1', 'original_v1')),
...
-- golang-migrate writes target=72, dirty=true before executing the down body.
-- Reject that metadata write while a 000073-only product exists so version 73
-- remains clean and the schema is unchanged.
DROP TRIGGER IF EXISTS trg_backup_asset_plain_text_content_downgrade_admission;
...
BEGIN
    SELECT RAISE(ABORT, '000073 downgrade blocked: plain_text/text_v2 delivery grant exists');
END;
