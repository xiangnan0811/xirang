...
CREATE TABLE recovery_point_rclone_native_versions (
    ...
    evidence_role VARCHAR(16) NOT NULL CHECK (evidence_role IN ('owned', 'reference')),
    ordinal BIGINT NOT NULL CHECK (ordinal >= 0),
    repository_id VARCHAR(32) NOT NULL,
    identity_digest VARCHAR(64) NOT NULL CHECK (length(identity_digest) = 64),
...
FOR EACH ROW
EXECUTE FUNCTION rclone_native_version_evidence_downgrade_admission();
