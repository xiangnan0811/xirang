...
func TestFoundationProcessingConfigUsesOneAtomicSnapshot(t *testing.T) {
	values := cloneFoundationTestValues(staticFoundationDefaults)
	for key, value := range map[string]string{
		"backup_assets.enabled":                                    "true",
		"backup_assets.processing_queue_max":                       "8000",
		"backup_assets.processing_interactive_slots":               "3",
		"backup_assets.processing_background_slots":                "4",
		"backup_assets.processing_pull_lease":                      "120s",
		"backup_assets.processing_pull_heartbeat":                  "30s",
		"backup_assets.processing_attempt_timeout":                 "3h",
		"backup_assets.processing_retry_max":                       "7",
		"backup_assets.processing_retry_base":                      "10s",
		"backup_assets.processing_retry_max_delay":                 "20m",
		"backup_assets.processing_input_request_max_bytes":         "33554432",
		"backup_assets.processing_input_cumulative_max_bytes":      "1073741824",
		"backup_assets.processing_input_max_requests":              "400",
		"backup_assets.processing_input_max_in_flight":             "3",
		"backup_assets.processing_sink_max_artifacts":              "16",
		"backup_assets.processing_sink_artifact_max_bytes":         "268435456",
		"backup_assets.processing_sink_total_max_bytes":            "805306368",
		"backup_assets.processing_protocol_json_max_bytes":         "32768",
		"backup_assets.processing_secret_classify":                 "true",
		"backup_assets.processing_backfill_paused":                 "false",
		"backup_assets.processing_backfill_batch_size":             "250",
		"backup_assets.processing_backfill_jobs_per_hour":          "2400",
		"backup_assets.processing_backfill_bytes_per_hour":         "21474836480",
		"backup_assets.processing_backfill_provider_concurrency":   "3",
		"backup_assets.processing_backfill_capability_concurrency": "2",
		"backup_assets.processing_backfill_recent_window":          "336h",
		"backup_assets.processing_backfill_history_aging_step":     "12h",
		"backup_assets.worker_local_enabled":                       "true",
		"backup_assets.worker_local_socket":                        "/run/xirang/asset-worker-test.sock",
		"backup_assets.worker_remote_enabled":                      "false",
		"backup_assets.worker_updater_enabled":                     "true",
		"backup_assets.worker_updater_online_enabled":              "true",
		"backup_assets.worker_updater_online_origins":              "https://bundles.example.internal:443,https://signatures.example.internal:443",
		"backup_assets.derived_store_root":                         "/var/lib/xirang-asset-runtime/test-derived",
		"backup_assets.derived_store_chunk_bytes":                  "524288",
		"backup_assets.derived_store_blob_max_bytes":               "2147483648",
		"backup_assets.derived_store_global_max_bytes":             "68719476736",
		"backup_assets.derived_store_reconcile_interval":           "10m",
		"backup_assets.derived_store_reconcile_batch_size":         "128",
	} {
		values[key] = value
	}
	reader := &snapshotSettingsReader{values: values}
	config, err := NewFoundationService(reader).ProcessingConfig()
	if err != nil {
		t.Fatalf("ProcessingConfig: %v", err)
	}
	if !config.Enabled || config.QueueMax != 8000 || config.InteractiveSlots != 3 || config.BackgroundSlots != 4 ||
		config.PullLease != 120*time.Second || config.PullHeartbeat != 30*time.Second || config.AttemptTimeout != 3*time.Hour ||
		config.RetryMax != 7 || config.RetryBase != 10*time.Second || config.RetryMaxDelay != 20*time.Minute {
		t.Fatalf("ProcessingConfig coordinator/retry=%+v", config)
	}
	if config.Input.RequestMaxBytes != 33554432 || config.Input.CumulativeMaxBytes != 1073741824 ||
		config.Input.MaxRequests != 400 || config.Input.MaxInFlight != 3 {
		t.Fatalf("ProcessingConfig input=%+v", config.Input)
	}
	if config.Sink.MaxArtifacts != 16 || config.Sink.ArtifactMaxBytes != 268435456 || config.Sink.TotalMaxBytes != 805306368 ||
		config.ProtocolJSONMaxBytes != 32768 {
		t.Fatalf("ProcessingConfig sink/protocol=%+v", config)
	}
	if !config.SecretClassify || config.Backfill.Paused || config.Backfill.BatchSize != 250 || config.Backfill.JobsPerHour != 2400 ||
		config.Backfill.BytesPerHour != 21474836480 || config.Backfill.ProviderConcurrency != 3 ||
		config.Backfill.CapabilityConcurrency != 2 || config.Backfill.RecentWindow != 336*time.Hour ||
		config.Backfill.HistoryAgingStep != 12*time.Hour {
		t.Fatalf("ProcessingConfig policy=%+v secret=%v", config.Backfill, config.SecretClassify)
	}
	if !config.Updater.Enabled || !config.Updater.OnlineEnabled || !reflect.DeepEqual(config.Updater.OnlineOrigins, []string{
		"https://bundles.example.internal:443", "https://signatures.example.internal:443",
	}) {
		t.Fatalf("ProcessingConfig updater=%+v", config.Updater)
	}
	if !config.LocalWorker.Enabled || config.LocalWorker.Socket != "/run/xirang/asset-worker-test.sock" || config.RemoteWorker.Enabled ||
		config.RemoteWorker.ListenAddress != "" || config.RemoteWorker.ServerCertFile != "" || config.RemoteWorker.ServerKeyFile != "" ||
		config.RemoteWorker.ClientCAFile != "" || config.RemoteWorker.TrustDomain != "" {
		t.Fatalf("ProcessingConfig transports=%+v %+v", config.LocalWorker, config.RemoteWorker)
	}
	if config.DerivedStore.Root != "/var/lib/xirang-asset-runtime/test-derived" || config.DerivedStore.ChunkBytes != 524288 ||
		config.DerivedStore.BlobMaxBytes != 2147483648 || config.DerivedStore.GlobalMaxBytes != 68719476736 ||
		config.DerivedStore.ReconcileInterval != 10*time.Minute || config.DerivedStore.ReconcileBatchSize != 128 {
		t.Fatalf("ProcessingConfig derived=%+v", config.DerivedStore)
	}
	if reader.effectiveReads != 0 || reader.snapshotReads != 1 {
		t.Fatalf("ProcessingConfig mixed per-key reads: effective=%d snapshot=%d", reader.effectiveReads, reader.snapshotReads)
	}
}

func TestFoundationProcessingConfigRequiresCompleteAtomicSnapshot(t *testing.T) {
	if _, err := NewFoundationService(staticSettingsReader{}).ProcessingConfig(); !errors.Is(err, ErrInvalidState) {
		t.Fatalf("reader without snapshot port got %v, want ErrInvalidState", err)
	}
	reader := &snapshotSettingsReader{values: cloneFoundationTestValues(staticFoundationDefaults)}
	delete(reader.values, "backup_assets.processing_pull_lease")
	if _, err := NewFoundationService(reader).ProcessingConfig(); !errors.Is(err, ErrInvalidState) {
		t.Fatalf("incomplete processing snapshot got %v, want ErrInvalidState", err)
	}
}
...
var staticFoundationDefaults = map[string]string{
	...
	"backup_assets.processing_queue_max":                       "10000",
	"backup_assets.processing_interactive_slots":               "2",
	"backup_assets.processing_background_slots":                "2",
	"backup_assets.processing_pull_lease":                      "90s",
	"backup_assets.processing_pull_heartbeat":                  "20s",
	"backup_assets.processing_attempt_timeout":                 "2h",
	"backup_assets.processing_retry_max":                       "5",
	"backup_assets.processing_retry_base":                      "5s",
	"backup_assets.processing_retry_max_delay":                 "15m",
	"backup_assets.processing_input_request_max_bytes":         "67108864",
	"backup_assets.processing_input_cumulative_max_bytes":      "2147483648",
	"backup_assets.processing_input_max_requests":              "512",
	"backup_assets.processing_input_max_in_flight":             "4",
	"backup_assets.processing_sink_max_artifacts":              "32",
	"backup_assets.processing_sink_artifact_max_bytes":         "536870912",
	"backup_assets.processing_sink_total_max_bytes":            "1073741824",
	"backup_assets.processing_protocol_json_max_bytes":         "65536",
	...
	"backup_assets.worker_local_enabled":                       "false",
	"backup_assets.worker_local_socket":                        "/run/xirang/asset-worker.sock",
	"backup_assets.worker_remote_enabled":                      "false",
	...
	"backup_assets.derived_store_root":                         "/var/lib/xirang-asset-runtime/derived",
	"backup_assets.derived_store_chunk_bytes":                  "1048576",
	"backup_assets.derived_store_blob_max_bytes":               "4294967296",
