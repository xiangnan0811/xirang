...
func (service *Service) loadFileSourcePoints(ctx context.Context, scope AuthorizationScope) ([]model.RecoveryPoint, error) {
	...
	if err := service.db.WithContext(ctx).Select(
		...
		"committed_at", "entry_count", "logical_bytes", "physical_availability", "created_at",
	).Where("id IN ?", authorized).Find(&points).Error; err != nil {
...
func (service *Service) composeFileSourceProjection(ctx context.Context, points []model.RecoveryPoint) (fileSourceProjection, error) {
	...
	tasks, err := service.loadFileSourceTasks(ctx, points)
	if err != nil {
	...
	for _, point := range points {
		...
		if err := validateFileSourcePoint(point, repository); err != nil {
			return fileSourceProjection{}, err
		}
		nodeID, err := fileSourceNodeID(point)
		if err != nil {
		...
		if !nodeExists {
			return fileSourceProjection{}, fmt.Errorf("%w: file-source node changed", ErrCatalogUnavailable)
		}
		key, label, err := fileSourceSetCoordinates(point, tasks, nodeID)
		if err != nil {
			return fileSourceProjection{}, err
		}
		...
		sets[key] = append(sets[key], fileSourcePoint{
			point: point, repository: repository, setID: setID, setLabel: label, coverage: coverage[point.ID],
		})
	}
	...
	for key, grouped := range sets {
		...
		for _, item := range grouped {
			version := FileSourceVersionDTO{
				...
				ProducingTaskID: item.point.ProducingTaskID, CapturedAt: utcPointer(item.point.CapturedAt),
				...
				ContentAvailability: contentAvailability(item.point, item.repository), EntryCount: item.point.EntryCount,
	...
	for nodeID, items := range nodeSets {
		...
		dto := FileSourceNodeDTO{
			NodeID: nodeID, DisplayName: strings.TrimSpace(node.Name), BackupSetCount: len(items),
...
func (service *Service) loadFileSourceTasks(ctx context.Context, points []model.RecoveryPoint) (map[uint]fileSourceTaskControl, error) {
	...
	if len(result) != len(ids) {
		return nil, fmt.Errorf("%w: file-source task changed", ErrCatalogUnavailable)
	}
	return result, nil
}
...
func (service *Service) loadFileSourceNodes(ctx context.Context, ids map[uint]struct{}) (map[uint]fileSourceNodeControl, error) {
	...
	if len(result) != len(ids) {
		return nil, fmt.Errorf("%w: file-source node changed", ErrCatalogUnavailable)
	}
	return result, nil
}
...
func (service *Service) loadFileSourceRepositories(ctx context.Context, points []model.RecoveryPoint) (map[string]model.BackupRepository, error) {
	...
	if err := service.db.WithContext(ctx).Select("id", "status").Where("id IN ?", ids).Find(&rows).Error; err != nil {
...
func fileSourceSetCoordinates(
	point model.RecoveryPoint,
	tasks map[uint]fileSourceTaskControl,
	nodeID uint,
) (fileSourceSetKey, string, error) {
	if backupasset.PointVersionSemantics(point.Semantics) != backupasset.PointImportedBaseline && point.ProducingTaskID != nil {
		task, exists := tasks[*point.ProducingTaskID]
		if !exists {
			return fileSourceSetKey{}, "", fmt.Errorf("%w: file-source task lineage unavailable", ErrCatalogUnavailable)
		}
		label, err := sanitizeFileSourceLabel(task.Name)
		if err != nil {
			return fileSourceSetKey{}, "", err
		}
		return fileSourceSetKey{kind: FileSourceLineageTask, nodeID: nodeID, taskID: task.ID}, label, nil
	}
	...
	return fileSourceSetKey{kind: FileSourceLineageImported, nodeID: nodeID, repositoryID: point.RepositoryID}, "Imported backup", nil
}
...
func validateFileSourcePoint(point model.RecoveryPoint, repository model.BackupRepository) error {
	if backupasset.ValidateOpaqueID(point.ID) != nil || backupasset.ValidateOpaqueID(point.RepositoryID) != nil ||
		point.RepositoryID != repository.ID || !publicCatalogPoint(point) || point.CreatedAt.IsZero() ||
		...
		return fmt.Errorf("%w: invalid file-source point", ErrUnknownInternalState)
	}
	return nil
}
...
func (item FileSourceNodeDTO) validate() error {
	if item.NodeID == 0 || item.BackupSetCount <= 0 || !validCoverageStatuses[item.CatalogCoverage] ||
...
func (item FileSourceBackupSetDTO) validate() error {
	if backupasset.ValidateOpaqueID(item.BackupSetID) != nil || item.NodeID == 0 || item.VersionCount <= 0 ||
		...
		!validCoverageStatuses[item.CatalogCoverage] || (item.LatestRetainedAt != nil && !isUTC(*item.LatestRetainedAt)) {
...
func (item FileSourceRecoveryPointDTO) validate() error {
	if item.NodeID == 0 || backupasset.ValidateOpaqueID(item.BackupSetID) != nil ||
		...
		(item.ProducingTaskID != nil && *item.ProducingTaskID == 0) {
...
func validFileSourceLifecycleState(state backupasset.RecoveryPointState) bool {
	return state == backupasset.RecoveryPointObserved || state == backupasset.RecoveryPointCommitted || state == backupasset.RecoveryPointDegraded
}
