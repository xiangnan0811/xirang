...
func TestDerivedReferencesAreSourceLocalAndSharedAcrossRecoveryPoints(t *testing.T) {
	...
	if err := harness.lifecycle.RevokeSet(context.Background(), first.setID, DerivedRevokeExpired); err != nil {
...
func TestDerivedRevokeCommitsProjectionBeforeReferenceKeyAndBlobDestruction(t *testing.T) {
	...
	harness.projection.onRevoke = func(request DerivedProjectionRevoke) error {
	...
	if err := harness.lifecycle.RevokeSet(context.Background(), reference.setID, DerivedRevokeExpired); err != nil {
...
func TestDerivedRevokeStopsBeforeMutationWhenProjectionRevokeFails(t *testing.T) {
	...
	harness.projection.onRevoke = func(DerivedProjectionRevoke) error { return errors.New("search unavailable") }
	if err := harness.lifecycle.RevokeSet(context.Background(), reference.setID, DerivedRevokeExpired); err == nil {
...
func TestDerivedKeyLossRevokesProjectionBeforeMarkingKeyLost(t *testing.T) {
	...
	harness.projection.onRevoke = func(DerivedProjectionRevoke) error {
		...
		if err := harness.db.First(&persisted, "artifact_id = ?", reference.authorization.ArtifactID).Error; err != nil {
			return err
		}
...
func newDerivedLifecycleHarness(t *testing.T) *derivedLifecycleHarness {
	...
	projection := &derivedProjectionFake{}
	lifecycle, err := NewDerivedLifecycle(storeHarness.db, storeHarness.store, projection, storeHarness.store.now)
	if err != nil {
		t.Fatal(err)
	}
...
func (*derivedProjectionFake) Publish(_ context.Context, request DerivedProjectionPublish) (DerivedProjectionPublication, error) {
	return DerivedProjectionPublication{ArtifactSetID: request.ArtifactSetID, Revision: 1}, nil
}

func (fake *derivedProjectionFake) Revoke(_ context.Context, request DerivedProjectionRevoke) error {
	fake.revocations++
	if fake.onRevoke != nil {
		return fake.onRevoke(request)
	}
	return nil
}
